...
public class IntervalAssertions : ReferenceTypeAssertions<Interval, IntervalAssertions>
{
    ...
    {
        return ExecuteAssertion(Subject != expected, "to not be", expected, because, becauseArgs);
    }
    ...
    private AndConstraint<IntervalAssertions> ExecuteAssertion(
        bool condition, string description, Interval expected, string? because = null, params object[] becauseArgs
    )
    {
        Execute.Assertion
