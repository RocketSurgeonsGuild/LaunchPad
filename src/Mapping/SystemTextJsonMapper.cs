...
public partial class SystemTextJsonMapper(IOptionsMonitor<JsonSerializerOptions> options)
{
    ...
    {
        return source is { ValueKind: not JsonValueKind.Undefined, }
    ...
    {
        return source switch { null or { Length: 0, } => new(), _ => JsonSerializer.Deserialize<JsonElement>(source, options.CurrentValue), };
    }
    ...
    {
        return source switch { null => null, { Length: 0, } => new(), _ => JsonSerializer.Deserialize<JsonElement>(source, options.CurrentValue), };
    }
