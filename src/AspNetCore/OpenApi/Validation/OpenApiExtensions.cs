...
public static class OpenApiExtensions
{
    ...
    internal static void SetNewMin(
        this OpenApiSchema schemaProperty, Expression<Func<OpenApiSchema, int?>> prop, int? newValue, bool setNotNullableIfMinLengthGreaterThenZero = true
    )
    {
        if (newValue.HasValue)
        {
    ...
    internal static void SetNewMin(
        this OpenApiSchema schemaProperty, Expression<Func<OpenApiSchema, decimal?>> prop, decimal? newValue,
        bool setNotNullableIfMinLengthGreaterThenZero = true
    )
    {
        if (newValue.HasValue)
        {
    ...
    private static int NewMaxValue(int? current, int newValue)
    {
        return current.HasValue ? Math.Min(current.Value, newValue) : newValue;
    }

    private static decimal NewMaxValue(decimal? current, decimal newValue)
    {
        return current.HasValue ? Math.Min(current.Value, newValue) : newValue;
    }

    private static int NewMinValue(int? current, int newValue)
    {
        return current.HasValue ? Math.Max(current.Value, newValue) : newValue;
    }

    private static decimal NewMinValue(decimal? current, decimal newValue)
    {
        return current.HasValue ? Math.Max(current.Value, newValue) : newValue;
    }
