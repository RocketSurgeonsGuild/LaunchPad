...
public class SchemaGenerationTests : UnitTestBase
{
    ...
    public void integer_property_should_not_be_nullable()
    {
        ...
        void ShouldBeSuccess(NumberEntity entity)
        {
            new NumberEntity.Validator().ValidateAndThrow(entity);
        }

        void ShouldBeFailed(NumberEntity entity)
        {
            new NumberEntity.Validator().Validate(entity).IsValid.Should().BeFalse();
        }
    ...
    public void TextNullability()
    {
        ...
        new SchemaBuilder<TestEntity>()
           .AddRule(
                entity => entity.NullableTextValue,
                rule => rule.MaximumLength(5),
                schema => schema.Nullable.Should().Be(true)
            );
    }
    ...
    public void MinimumLength_ShouldNot_Set_Nullable_By_Default()
    {
        ...
        new SchemaBuilder<TestEntity>()
           .AddRule(
                entity => entity.TextValue, rule => rule.MinimumLength(1), schema =>
                {
