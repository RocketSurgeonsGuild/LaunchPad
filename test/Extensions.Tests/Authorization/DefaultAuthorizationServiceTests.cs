...
public class DefaultAuthorizationServiceTests
{
    ...
    public async Task Authorize_ShouldAllowIfClaimIsPresent()
    {
        ...
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Basic", policy => policy.RequireClaim("Permission", "CanViewPage")); }); }
        );
        var user = new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim("Permission", "CanViewPage") }));
    ...
    public async Task Authorize_ShouldAllowIfClaimIsPresentWithSpecifiedAuthType()
    {
        ...
        var user = new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim("Permission", "CanViewPage") }, "Basic"));
    ...
    public async Task Authorize_ShouldAllowIfClaimIsAmongValues()
    {
        ...
        var authorizationService = BuildAuthorizationService(
            ...
            {
                services.AddAuthorization(
                    options => { options.AddPolicy("Basic", policy => policy.RequireClaim("Permission", "CanViewPage", "CanViewAnything")); }
                );
            }
        );
    ...
    public async Task Authorize_ShouldInvokeAllHandlersByDefault()
    {
        ...
        var authorizationService = BuildAuthorizationService(
            services =>
            {
                ...
                services.AddAuthorization(options => { options.AddPolicy("Custom", policy => policy.Requirements.Add(new CustomRequirement())); });
            }
        );
    ...
    public async Task Authorize_PolicyCanAuthenticationSchemeWithNameClaim()
    {
        ...
        var authorizationService = BuildAuthorizationService();
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(new[] { new Claim(ClaimTypes.Name, "Name") }, "AuthType")
        );
    ...
    public async Task RolePolicyCanRequireSingleRole()
    {
        ...
        var authorizationService = BuildAuthorizationService();
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(new[] { new Claim(ClaimTypes.Role, "Admin") }, "AuthType")
        );
    ...
    public async Task RolePolicyCanRequireOneOfManyRoles()
    {
        ...
        var authorizationService = BuildAuthorizationService();
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(new[] { new Claim(ClaimTypes.Role, "Users") }, "AuthType")
        );
    ...
    public async Task RolePolicyCanBlockNoRole()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Basic", policy => policy.RequireRole("Admin", "Users")); }); }
        );
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(
                new Claim[]
    ...
    public void PolicyThrowsWithNoRequirements()
    {
        Assert.Throws<InvalidOperationException>(
            () => BuildAuthorizationService(services => { services.AddAuthorization(options => { options.AddPolicy("Basic", policy => { }); }); })
        );
    }
    ...
    public async Task RequireUserNameFailsForWrongUserName()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Hao", policy => policy.RequireUserName("Hao")); }); }
        );
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(
                new[]
    ...
    public async Task CanRequireRoleWithDiffClaimType()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Hao", policy => policy.RequireRole("Hao")); }); }
        );
        var identity = new ClaimsIdentity("AuthType", "Name", "Role");
    ...
    public async Task CanApproveAnyAuthenticatedUser()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Any", policy => policy.RequireAuthenticatedUser()); }); }
        );
        var user = new ClaimsPrincipal(new ClaimsIdentity());
        user.AddIdentity(
            new ClaimsIdentity(
    ...
    public async Task CustomReqWithNoHandlerFails()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Custom", policy => policy.Requirements.Add(new CustomRequirement())); }); }
        );
        var user = new ClaimsPrincipal();
    ...
    public async Task CanAuthorizeAllSuperuserOperations()
    {
        ...
        var authorizationService = BuildAuthorizationService(
            services =>
            {
                services.AddSingleton<IAuthorizationHandler>(new ExpenseReportAuthorizationHandler(new[] { Operations.Edit }));
    ...
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(services => { services.AddTransient<IAuthorizationHandler, EvenHandler>(); });
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(
                new Claim[]
                {
    ...
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(services => { services.AddTransient<IAuthorizationHandler, NotCalledHandler>(); });
        var user = new ClaimsPrincipal(
            new ClaimsIdentity(
                new[]
                {
    ...
    public async Task CanAuthorizeOnlyAllowedOperations()
    {
        // Arrange
        var authorizationService = BuildAuthorizationService(
            services => { services.AddSingleton<IAuthorizationHandler>(new ExpenseReportAuthorizationHandler(new[] { Operations.Edit })); }
        );
        var user = new ClaimsPrincipal();
    ...
    {
        var authorizationService = BuildAuthorizationService(
            services => { services.AddAuthorization(options => { options.AddPolicy("Basic", policy => policy.RequireAssertion(context => true)); }); }
        );
        var user = new ClaimsPrincipal();
    ...
    public async Task CanAuthorizeWithAsyncAssertionRequirement()
    {
        var authorizationService = BuildAuthorizationService(
            services =>
            {
                services.AddAuthorization(options => { options.AddPolicy("Basic", policy => policy.RequireAssertion(context => Task.FromResult(true))); });
            }
        );
        var user = new ClaimsPrincipal();
    ...
    public async Task CanReplaceDefaultPolicyProvider()
    {
        var authorizationService = BuildAuthorizationService(
            services =>
            {
                ...
                services.AddAuthorization(options => { options.AddPolicy("Basic", policy => policy.RequireAssertion(context => true)); });
            }
        );
        var user = new ClaimsPrincipal();
    ...
    public async Task Authorize_ShouldLogRequirementDetailWhenUnHandled()
    {
        ...
        static void Assertion(LogLevel level, EventId eventId, object state, Exception exception, Func<object, Exception, string> formatter)
        {
            ...
            Assert.Equal(
                "Authorization failed. These requirements were not met:" + Environment.NewLine + "LogRequirement" + Environment.NewLine + "LogRequirement",
                message
            );
        }
    ...
    private class ReasonableFailHandler : IAuthorizationHandler
    {
        ...
        public ReasonableFailHandler(string reason)
        {
            _reason = reason;
        }
    ...
    public async Task PassThroughRequirementWillSucceedWithoutCustomHandler(bool shouldSucceed)
    {
        ...
        var authorizationService = BuildAuthorizationService(
            ...
            {
                services.AddAuthorization(
                    options => { options.AddPolicy("Passthrough", policy => policy.Requirements.Add(new PassThroughRequirement(shouldSucceed))); }
                );
            }
        );
        var user = new ClaimsPrincipal();
    ...
    public class ExpenseReportAuthorizationHandler : AuthorizationHandler<OperationAuthorizationRequirement, ExpenseReport>
    {
        ...
        protected override Task HandleRequirementAsync(
            AuthorizationHandlerContext context, OperationAuthorizationRequirement requirement, ExpenseReport resource
        )
        {
